...
function map.canmove(x, y)
	return not map.stops[x][y]
end

function map.move(actor)
	camera.move(actor.x, actor.y)
end

function map.setactorposition(actor)
	local x, y = camera.getposincamera(actor.x, actor.y)
...
function map.update(actor)
	local x, y = camera.getcamerarect()
	local tempx, tempy = - (x + actor.offsetx), - (y + actor.offsety)
